...
class AppSubmenu extends Component {
    ...
    renderLink(item, i) {
        ...
        if (item.to) {
            ...
            target = { item.target } > { content } </NavLink>)
        } else {
            return ( <a href = { item.url }
                onClick = {
                    (e) => this.onMenuItemClick(e, item, i)
                }
                target = { item.target } > { content } </a>
            )
        }
    }
        ...
        const items =
            ...
            this.props.items.map((item, i) => {
                    ...
                    return ( <li className = { styleClass }
                        key = { i } > {
                            item.items && this.props.root === true && < div className = "arrow" > </div>} { this.renderLink(item, i) } <
    ...
    export class AppMenu extends Component {
        ...
        render() {
            return ( <div className = "layout-menu-container" >
                <
                AppSubmenu items = { this.props.model }
